...
public class InetAddressUtil {
		...
		try{
			if(StringUtils.isEmpty(localIp) ){ //如果localIp为空
				...
				try {
					...
					localIp = ConverUtils.Obj2Str(System.getProperty("server.address"),"");
					if(StringUtils.isEmpty(localIp)) {
						//如果环境变量中没有server.address重新读取文件
						localIp = readBindIp();
					}else {
						logger.info("InetAddressUtil read bind ip=" + localIp + " from file[sigmam.properties] key[server.address]");
						return localIp;
					}
				}catch(Exception ex) {
				}
	...
	/**
	 * 从sigmam.properties 获取 server.address属性
	 * @return
	 */
	public static String readBindIp() {
		...
		try {
			inputStream = InetAddressUtil.class.getClassLoader().getResourceAsStream("sigmam.properties");
			Properties properties = new Properties();
			properties.load(inputStream);
			bindIp_fromFile = properties.getProperty("server.address");
			if(bindIp_fromFile != null) {
				bindIp_fromFile = bindIp_fromFile.trim();
			}
			System.setProperty("server.address", bindIp_fromFile);
			inputStream.close();
		}catch(Exception e) {
			///logger.warn("Load bindIp from server file[sigmam.properties] exception!", e.getMessage());
		} finally {
			try {
				if(inputStream != null) {
					inputStream.close();

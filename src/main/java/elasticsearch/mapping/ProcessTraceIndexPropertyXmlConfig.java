 ...
 */
public class ProcessTraceIndexPropertyXmlConfig extends IndexPropertyXmlConfig{
	// 日志
	private static Logger log = LoggerFactory.getLogger(ProcessTraceIndexPropertyXmlConfig.class);

	public static final String CONFIG_FILE = "es_process_tracerecord_mapping.xml";

	private static ProcessTraceIndexPropertyXmlConfig instance = null;

	public static ProcessTraceIndexPropertyXmlConfig getInstance() {
		if (instance == null) {
			synchronized (ProcessTraceIndexPropertyXmlConfig.class) {
				if (instance == null) {
					instance = new ProcessTraceIndexPropertyXmlConfig();
				}
			}
		}
		return instance;
	...
	private ProcessTraceIndexPropertyXmlConfig() {
        ...
        public void onMessage(String msg) {
					...
					try {
						...
						if (msgObject.getType() == MessageType.ES_INDEX_FIELD_UPDATE) {
							...
							if (ESMappingManager.ESMAPPING_PROCESS_TRACE.equalsIgnoreCase(obj + "")) {
			...
			log.error("Start message listener error", e);
		}
